...
O modelo da simulação define o sistema gerenciado como um sistema distribuído. Esse sistema distribuído foi modelado através de um grafo DS(P;C). O grafo DS é composto por um conjunto finito de n > 1 processos, denominado, P = {p1, p2,...,pn}, e um conjunto finito de canais C = f(Pi; Pj) | Pi,Pj E P. Vértices representam processos e arestas representam canais de comunicação. Cada canal de comunicação é uma abstração de toda a infraestrutura de comunicação entre dois processos e modela a interação entre os processos associados por meio de troca de mensagens. A arquitetura proposta adiciona ao modelo anteriormente um sistema controlador. Temos então que o sistema distribuído passa a ser representado pelo grafo DS(P;C;T), onde T é o sistema controlador. A arquitetura do sistema controlador T implementada através do padrão \textit{Regional Planner} é função sobretudo do tamanho das regiões R: T(R). Optou-se por definir um tamanho R constante para as regiões. Tendo em vista que o tamanho das regiões é constante temos que o número de regiões Nr é Nr = P/R. Por fim o modelo da simulação define o conceito de Job J. O custo de execução do Job J(R,D) é função do tamanho da região R e do nível de perturbação do ambiente D. O modelo proposto define a pertubação D através de uma variável aleatória. A frequência de ocorrência da perturbação também é uma variável aleatória.
